...
public class ConfigurationJPanel extends JPanel implements ChangeListener, ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == buttonPanel.getImportBtn()) {
        	...
        	if (index == 0) {
        		drawingJPanel.getScene().setSensorData(null);
        	} else {
    ...
    private SensorData generateRandomData() {
    	...
    	SensorData existingData = drawingJPanel.getScene().getSensorData();
    	ArrayList<Double> timestamps = existingData.getTimestamps();
    	double maxTimestamp = timestamps.get(timestamps.size() - 1);
    	
    	double timeStep = maxTimestamp / 999.0;									
    	
    	for (int i = 0; i < 1000; i++) {
    		double timestamp = i * timeStep;									// Even distribution
    		randomPoint = RandomNumber.between(-100, 100) / 100.0;				// My RandomNumber class can only generate integers, and an int from -1 to 1 will always be almost the same
